package edu.ftcphoenix.fw2.gamepad.rev;

import com.qualcomm.robotcore.hardware.Gamepad;

import edu.ftcphoenix.fw2.gamepad.GamepadInputs;
import edu.ftcphoenix.fw2.gamepad.GamepadKeys;
import edu.ftcphoenix.fw2.gamepad.InputButton;
import edu.ftcphoenix.fw2.gamepad.InputInterval;
...
public class GamepadController {
    ...
    private final double errorLeftX;
    private final double errorLeftY;
    private final double errorRightX;
    private final double errorRightY;
    ...
    public GamepadController(Gamepad gamepad, GamepadInputs gamepadInputs,
        ...
        // Get the starting values of the stick in case they are non-zero.
        //    This will be used to correct the stick values when reading
        //    the values.
        errorLeftX = this.gamepad.left_stick_x;
        errorLeftY = this.gamepad.left_stick_y;
        errorRightX = this.gamepad.right_stick_x;
        errorRightY = this.gamepad.right_stick_y;
    }

    public void addTelemetry(org.firstinspires.ftc.robotcore.external.Telemetry telemetry) {
        telemetry.addData("errorLeftX", errorLeftX);
        telemetry.addData("errorLeftY", errorLeftY);
        telemetry.addData("errorRightX", errorRightX);
        telemetry.addData("errorRightY", errorRightY);
    }
    ...
    /**
     * Error correction for joysticks on the gamepad. Sometimes the hardware
     * has a non-zero value even when the stick is not moved; we correct
     * for this.
     *
     * @param pos The actual stick value.
     * @param error The value of the stick with no usage.
     * @return The corrected value.
     */
    double correctStickPos(double pos, double error) {
        double newpos;
        if(pos >= error) {
            newpos = (pos - error) / (1 - error);
        }
        else {
            newpos = (pos - error) / (1 + error);
        }
    ...
    double getValue(GamepadKeys.Stick stick) {
        switch (stick) {
            case LEFT_STICK_X:
                return correctStickPos(gamepad.left_stick_x, errorLeftX);
            case LEFT_STICK_Y:
                // Invert the sign of the y stick values to make it consistent with
                //    normal geometric axes people think about.
                return correctStickPos(-gamepad.left_stick_y, errorLeftY);
            case RIGHT_STICK_X:
                return correctStickPos(gamepad.right_stick_x, errorRightX);
            case RIGHT_STICK_Y:
                // Invert the sign of the y stick values to make it consistent with
                //    normal geometric axes people think about.
                return correctStickPos(-gamepad.right_stick_y, errorRightY);
            default:
