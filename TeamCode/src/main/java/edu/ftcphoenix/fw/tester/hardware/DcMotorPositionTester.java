package edu.ftcphoenix.fw.tester.hardware;
...
import edu.ftcphoenix.fw.tester.ui.HardwareNamePicker;
import edu.ftcphoenix.fw.tester.ui.IntTuner;
import edu.ftcphoenix.fw.tester.ui.ScalarTuner;
...
public final class DcMotorPositionTester extends BaseTeleOpTester {
    ...
    private void restoreOriginalSettings() {
        ...
        try { motor.setPower(0.0); } catch (Exception ignored) {}

        try { if (origDir != null) motor.setDirection(origDir); } catch (Exception ignored) {}
        try { if (origZpb != null) motor.setZeroPowerBehavior(origZpb); } catch (Exception ignored) {}
        try { if (origMode != null) motor.setMode(origMode); } catch (Exception ignored) {}
    }

    private static DcMotor.RunMode safeGetMode(DcMotor m) {
        try { return m.getMode(); } catch (Exception ignored) { return null; }
    }

    private static DcMotor.Direction safeGetDir(DcMotor m) {
        try { return m.getDirection(); } catch (Exception ignored) { return null; }
    }

    private static DcMotor.ZeroPowerBehavior safeGetZpb(DcMotor m) {
        try { return m.getZeroPowerBehavior(); } catch (Exception ignored) { return null; }
    }
    ...
    private void renderTelemetry() {
        ...
        try { cur = motor.getCurrentPosition(); } catch (Exception ignored) {}
        try { busy = motor.isBusy(); } catch (Exception ignored) {}
        try { mode = motor.getMode(); } catch (Exception ignored) {}
        try { dir = motor.getDirection(); } catch (Exception ignored) {}
